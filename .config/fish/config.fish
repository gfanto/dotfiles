...
alias autoremove "sudo apt-get autoremove -y --purge"
...
alias ansifilter "sed -e 's/\x1b\[[0-9;]*m//g'"
...
abbr gocov "go test ./... -coverpkg=./... -coverprofile=coverage.out && go tool cover -html=coverage.out"

function ts2iso -d "Convert timestamp to ISO format"
  if count $argv > /dev/null
    date -d @$argv[1] --iso-8601=seconds
  else
    echo "Not enought arguments, you must specify the timestamp"
  end
end
...
function fgh -d "Git stashes"
  argparse s/show d/drop a/apply -- $argv
  ...
  if set stash (git stash list | fzf --delimiter=":" --preview="git stash show -p {1} | delta --no-gitconfig")
    if set -q _flag_show
      ...
      echo $stash | awk -F ":" '{print $1}' | xargs git stash apply
    else
